...
bool ShouldGoAlone() {
  ...
  if (StraightFreePathLength({0,0}, local_target_) < 0.5) {
    return false;
  }
  return true;
}
...
bool ShouldPass() {
  const HumanStateMsg b = human_b_;
  const HumanStateMsg c = human_c_;
  const Vector2f pose_b = {human_b_.pose.x, human_b_.pose.y};
  const Vector2f pose_c = {human_c_.pose.x, human_c_.pose.y};
  // Either there are no other humans of interest, or they are behind
  if (pose_b.x() <= 0 && pose_c.x() <= 0.0) {
    return true;
  }
}
