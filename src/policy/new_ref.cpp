...
bool ShouldFollow() {
  ...
  const float kVelThreshold = 0.0;
  // cout << "A Velocity: " << closest_vel.norm() + vel_.norm() << endl;
  if (fabs(AngleDiff(goal_angle, closest_angle)) <=0.8 && distance.norm() > 0.1
      && (closest_vel.norm()) > kVelThreshold) {
    return true;
  }
  return false;
}
...
bool ShouldPass() {
    // If the closest human is not going in the right direction, or
    // not going fast enough, and there are no humans beyond it.
    const Vector2f human_a(human_a_.pose.x, human_a_.pose.y);
    const Vector2f human_b(human_b_.pose.x, human_b_.pose.y);
    const Vector2f human_c(human_c_.pose.x, human_c_.pose.y);
    const float a_b = (human_a - human_b).norm();
    const float a_c = (human_a - human_c).norm();
    const float c_b = (human_b - human_c).norm();
    // cout << "human_a: " << human_a.norm() << endl;
    // cout << "human_b: " << human_b.norm() << endl;
    // cout << "a_b: " << a_b << endl;
    // cout << "human_c: " << human_c.norm() << endl;
    // cout << "a_c: " << a_c << endl;
