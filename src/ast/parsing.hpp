...
class library_parsing_error : public std::runtime_error {
...
std::vector<AST::Example> WindowExamples(
    const std::vector<AST::Example>& examples, const int window_size);

std::vector<AST::Example> ReadExamples(
    const std::string& file, std::unordered_set<AST::Var>& vars,
    std::vector<std::pair<std::string, std::string>>* transitions);

AST::ast_ptr LoadJson(const std::string& file);
AST::Example JsonToExample(const nlohmann::json& example);

std::vector<AST::ast_ptr> LoadSketches(
